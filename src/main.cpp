...
// where download_model.sh puts the weights and the tokenizer
// TODO: use args to provide the path or smth
const std::string MODEL_DIR = ".cache/huggingface/download";
...
// A queued prompt: the token ids the engine runs on, plus the text they came from
// so finished answers can be printed next to the question that produced them.
struct Request
{
    std::string text;
...
int loadWeights(Weights &weights)
{
    ...
    {
        std::cout << "Can't open " << safetensors_path << " (download it with ./download_model.sh)\n";
...
{
    // load the tokenizer first, it's the cheapest thing to fail on
    Tokenizer tokenizer;
    ...
    // PROMPTS: plain text now, the tokenizer turns them into ids and back
    std::vector<std::string> prompt_texts;
    for (int arg = 1; arg < argc; ++arg)
    {
        prompt_texts.push_back(argv[arg]);
    }
    if (prompt_texts.empty())
    {
        prompt_texts = {"What is 2+2?", "Name a color.", "Say hello.", "Capital of France?"};
    }

    std::queue<Request> queue;
    for (const std::string &prompt_text : prompt_texts)
    {
        std::vector<int> prompt_tokens = tokenizer.encodeChatPrompt(prompt_text);
        if ((int)prompt_tokens.size() > MAX_PROMPT_LEN)
        {
            std::cout << "Skipping prompt (" << prompt_tokens.size() << " tokens > MAX_PROMPT_LEN "
                      << MAX_PROMPT_LEN << "): " << prompt_text << "\n";
            continue;
        }
        std::cout << "Prompt (" << prompt_tokens.size() << " tokens): " << prompt_text << "\n";
        queue.push({prompt_text, std::move(prompt_tokens)});
    }
    ...
    // what each slot is currently answering, so the finished text can be printed with it
    std::vector<std::string> slot_prompt_text(BATCH_SIZE);
    std::vector<std::string> slot_answer_text(BATCH_SIZE);
    ...
    for (int slot = 0; slot < is_slot_free.size() && !queue.empty(); ++slot)
    {
        ...
        slot_prompt_text[slot] = queue.front().text; // prefill pops the request, grab the text first
        slot_answer_text[slot].clear();
        ...
        // prefill already produced the first token of the answer
        slot_answer_text[slot] += tokenizer.decodeToken(generated_tokens[slot].back());
    }
    ...
    while (true) // exit condition irrelevant for now, since it's an inference server that's supposed to run foreveeer!!!
    {
        ...
        for (int slot = 0; slot < num_active_slots; ++slot)
        {
            ...
            std::cout << "Output token: " << (float)max_token << ", token index: " << std::to_string(max_token_idx)
                      << ", text: \"" << tokenizer.decodeToken(max_token_idx, false) << "\"" << std::endl;
#endif
            if (max_token_idx == END_OF_TEXT_TOKEN_ID || max_token_idx == EOT_ID_TOKEN_ID || current_prompt_len[active_slot] == MAX_SEQ_LEN - 1)
            {
                std::cout << "\n=== slot " << active_slot << " ===\n"
                          << "Prompt: " << slot_prompt_text[active_slot] << "\n"
                          << "Answer: " << slot_answer_text[active_slot] << "\n"
                          << std::flush;
            ...
            else
            {
                ...
                slot_answer_text[active_slot] += tokenizer.decodeToken(max_token_idx);
