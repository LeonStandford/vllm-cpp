...
The engine itself no longer needs that script: [src/tokenizer.cpp](src/tokenizer.cpp) is a C++ port of the same thing, reading Llama's `tokenizer.json` directly (the tiktoken-style split regex, the ByteLevel byte↔unicode alphabet, and the BPE merges). So `tiny-vllm` takes prompts as plain text and prints answers as plain text:
...
`python/tokenizer.py` stays around as the reference implementation to cross-check against.
